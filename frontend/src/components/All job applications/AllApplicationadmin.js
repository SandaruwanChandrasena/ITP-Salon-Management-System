...
function AllApplicationadmin() {
  ...
  const approveApplicant = async (id, email) => {
    try {
      ...
      if (result.isConfirmed) {
        ...
        setFilteredApplicants(prevApplicants => prevApplicants.filter(applicant => applicant._id !== id));
  ...
  const handleSearch = (e) => {
    ...
    const filteredData = applicants.filter(applicant =>
      applicant.position.toLowerCase().includes(e.target.value.toLowerCase())
    );
